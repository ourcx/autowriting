...
export async function smokeCandidates(base, token) {
  ...
  try {
    ...
    for (const response of [first, second]) assert.match(await response.text(), /event: done/)
    assert.equal(maxActive, 2)
    assert.equal((await json(root)).article, '# 原稿\n\n原稿不能被候选覆盖。')
    mode = 'length'
    assert.match(await (await stream(rows[2])).text(), /event: error/)
    let stored = await json(`${root}/candidates`)
    const partial = stored.find(row => row.id === rows[2].id)
    assert.equal(partial.status, 'interrupted')
    assert.ok(partial.content.includes('保留内容'))
    mode = 'normal'
    assert.match(await (await stream(rows[2])).text(), /event: done/)
    ...
    assert.equal((await json(`${root}/candidates`)).length, 3)
