...
> 不必一次改变所有工作，先让明天的一件事变得更轻松。`,materials:'',task:''};
 ...
 assert.equal(await paper.locator('img[src*="current-cover"], img[src*="old-cover"]').count(),0,'当前与历史封面不得自动追加到正文')
 ...
 assert.equal(await page.locator('.cs-template-card').count(), 19)
 ...
 await page.getByText('主题与模板',{exact:true}).click()
 // 新模板会自动组合开篇、章节和结尾素材，切换时无需人工逐个插入。
 await page.getByText('主题与模板',{exact:true}).click()
 await page.getByRole('button',{name:/共读书房/}).click()
 await page.getByRole('button',{name:'应用模板',exact:true}).click()
 assert.ok(await paper.locator('img[src*="book-stack"], img[src^="data:image/png"]').count())
 assert.ok(await paper.locator('img[src*="ink-quill"], img[src^="data:image/png"]').count())
 await page.getByRole('button',{name:'撤销',exact:true}).click()
 for (const name of ['森系来信','校园笔记','共读书房','春日花园','生日派对','新年特辑','旅行日记','咖啡生活','照片故事','品牌简报','知识卡片','夜读时光']) {
  await page.getByText('主题与模板',{exact:true}).click()
  await page.getByRole('button',{name:new RegExp(name)}).click()
  await page.getByRole('button',{name:'应用模板',exact:true}).click()
  assert.equal(await paper.locator('img').count(), 3, `${name} 应组合开篇、章节和结尾素材`)
  await page.getByRole('button',{name:'阅读预览',exact:true}).click()
  assert.deepEqual(await page.locator('.cs-reading-paper').evaluate(root => [root, ...root.querySelectorAll('*')]
    .filter(el => el.clientWidth > 0 && el.scrollWidth > el.clientWidth + 2)
    .map(el => el.tagName)), [], `${name} 不得横向溢出`)
  await page.getByRole('button',{name:'返回编辑',exact:true}).click()
  await page.getByRole('button',{name:'撤销',exact:true}).click()
 }
 // 选中章节后可用 10 种组件一键换样式，并且进入同一撤销历史。
 await paper.locator('h2').first().click()
 await page.getByText('章节组件',{exact:true}).click()
 assert.equal(await page.locator('.cs-section-preset-grid button').count(), 10)
 await page.screenshot({path:path.join(artifactDir, 'section-presets.png')})
 await page.getByRole('button',{name:'票券章节 适合活动流程',exact:true}).click()
 assert.ok(await paper.locator('[data-publication-frame="ticket"]').count())
 await page.getByRole('button',{name:'撤销',exact:true}).click()
 ...
 await page.waitForFunction(() => [...document.querySelectorAll('.cs-material-grid img')].every(img => img.complete && img.naturalWidth > 0))
 assert.equal(await page.locator('.cs-material-grid').first().locator('button').count(), 31)
 await page.getByRole('textbox',{name:'搜索装饰素材'}).fill('灯笼')
 assert.equal(await page.locator('.cs-material-grid').first().locator('button').count(), 1)
 await page.getByRole('textbox',{name:'搜索装饰素材'}).fill('')
 await page.getByRole('combobox',{name:'素材分类'}).selectOption('阅读装饰')
 assert.equal(await page.locator('.cs-material-grid').first().locator('button').count(), 3)
 await page.getByRole('combobox',{name:'素材分类'}).selectOption('全部')
 await page.screenshot({path:path.join(artifactDir,'materials.png')})
 ...
 await page.getByRole('button',{name:'爱心结尾线 结尾装饰',exact:true}).click()
 assert.equal(await page.getByRole('combobox',{name:'素材来源'}).inputValue(), 'heart-ending')
