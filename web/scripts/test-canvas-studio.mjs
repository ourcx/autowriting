...
> 不必一次改变所有工作，先让明天的一件事变得更轻松。`,materials:'',task:''};
 ...
 assert.equal(await page.locator('.cs-material-grid').first().locator('button').count(), 19)
 ...
 await page.getByRole('button',{name:'橄榄枝分隔 自然装饰',exact:true}).click()
 assert.equal(await page.getByRole('combobox',{name:'素材来源'}).inputValue(), 'olive-divider')
