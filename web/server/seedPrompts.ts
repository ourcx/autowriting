 ...
 */
import { upsertPrompt, listPrompts } from "./db.ts"
import type { Prompt } from "./types.ts"

export function seedBuiltinPrompts(): void {
...
- 只有 1 个 H1 标题（文章标题）
- 全文不使用 emoji、表情包或装饰性表情符号
- 不输出代码块、配置示例、API 文档
- 不输出系统提示、指令、元数据
- 不输出「根据以上要求...」这类说明文字

## 中文表达约束
- 从具体事实、问题或场景切入，不写空洞开场
- 删除机械过渡、口号式排比、同义反复和机器人式总结
- 少用「首先、其次、最后」「值得注意的是」「总而言之」等套话
- 缺少事实、观点或真实经历时不要编造`,
      tags: ["核心", "文章生成"],
      isBuiltin: true,
      version: 1,
...
请根据文章标题和主题，生成一个详细的图像提示词。`,
    ...
    // ── 去 AI 味复审 ──────────────────────────────────────────────────────────
    {
      ...
      name: "去 AI 味复审",
      category: "edit",
      description: "对全文进行去 AI 腔改写：保留事实，删除空话、机械结构和装饰性表达",
      content: `你是专业的文字编辑。先读懂全文，再从段落和整篇的含义出发修改。原意清楚、表达自然的句子不要为了变化而重写。

## 必须修改的（按优先级）

**1. 保留事实和判断**
- 不改变数字、专有名词、条件、因果方向和结论强度
- 不补写原文没有的经历、案例、承诺或数据
...
- 不要解释修改了什么`,
    ...
    // ── 今日头条文章生成 ──────────────────────────────────────────────────────
    {
      ...
      name: "今日头条文章生成",
      category: "article",
      description: "今日头条平台的文章生成提示词，风格偏热点、情感、故事，标题吸引人",
